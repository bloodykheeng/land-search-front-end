let cldcolumn = [
    { field: 'landsearchregistrar', headerName: 'landsearchregistrar', width: 150 },
    { field: 'Clin_Number', headerName: 'Clin_Number', width: 150 },
    { field: 'Object_Id', headerName: 'Object_Id', width: 150 },
    { field: 'Global_Id', headerName: 'Global_Id', width: 150 },
    { field: 'OwnershipType_Name', headerName: 'OwnershipType_Name', width: 130 },
    { field: 'Cla_Name', headerName: 'Cla_Name', width: 130 },
    { field: 'Cla_Certificate_Of_Incorporation_Number', headerName: 'Cla_Certificate_Of_Incorporation_Number', width: 130 },
    { field: 'Name_Of_The_Community', headerName: 'Name_Of_The_Community', width: 350 },
    { field: 'Region_Name', headerName: 'Region_Name', width: 130 },
    { field: 'District_Name', headerName: 'District_Name', width: 130 },
    { field: 'County_Name', headerName: 'County_Name', width: 130 },
    { field: 'SubCounty_Name', headerName: 'SubCounty_Name', width: 130 },
    { field: 'parish_Name', headerName: 'parish_Name', width: 130 },
    { field: 'Village_Name', headerName: 'Village_Name', width: 130 },
    { field: 'Plot_Number', headerName: 'Plot_Number', width: 130 },
    { field: 'Perimeter_Poly_Km', headerName: 'Perimeter_Poly_Km', width: 130 },
    { field: 'Area_Poly_Ha', headerName: 'Area_Poly_Ha', width: 130 },
    { field: 'Land_Use', headerName: 'Land_Use', width: 350  },
    { field: 'Easements_Or_Other_Persons_Rights', headerName: 'Easements_Or_Other_Persons_Rights', width: 350  },
    { field: 'Value_Per_Acre', headerName: 'Value_Per_Acre', width: 150 },
    { field: 'Year_Of_Evaluation', headerName: 'Year_Of_Evaluation', width: 150 },
    { field: 'Comments', headerName: 'Comments', width: 350  },
    { field: 'Date_of_the_Interview', headerName: 'Date_of_the_Interview', width: 150 },
    { field: 'Interviewed_By', headerName: 'Interviewed_By', width: 150 },
    { field: 'Status', headerName: 'Status of the Survey', width: 150 },
    { field: 'Supervision_Notes', headerName: 'Supervision_Notes', width: 150 },
    { field: 'Shape_Area', headerName: 'Shape_Area', width: 150 },
    { field: 'Shape_Length', headerName: 'Shape_Length', width: 150 },
    { field: 'CreationDate', headerName: 'CreationDate', width: 150 },
    { field: 'Creator_Name', headerName: 'Creator_Name', width: 150 },
    { field: 'EditDate', headerName: 'EditDate', width: 150 },
    { field: 'Editor_Name', headerName: 'Editor_Name', width: 150 },
    { field: 'Surveyed_Parish_Name', headerName: 'Surveyed_Parish_Name', width: 150 },
    { field: 'Minute_Number', headerName: 'Minute_Number', width: 150 },
    { field: 'Instrument_Number', headerName: 'Instrument_Number', width: 150 },
    { field: 'Recorders_Name', headerName: 'Recorders_Name', width: 150 },
    { field: 'regDateAndTime', headerName: 'regDateAndTime', width: 150 },
    { field: 'Land_Search_RegDate', headerName: 'Land_Search_RegDate', width: 150 },
    { field: 'excellfilepath', headerName: 'excellfilepath', width: 150 },
    { field: 'geoshapefilepath', headerName: 'geoshapefilepath', width: 150 }

] ;

let ownercolumn = [
    { field: 'username', headerName: 'landsearchregistrar', width: 150 },
    { field: 'Object_Id', headerName: 'Object_Id', width: 150 },
    { field: 'Global_Id', headerName: 'Global_Id', width: 150 },
    { field: 'Surname', headerName: 'Surname', width: 100 },
    { field: 'GivenName', headerName: 'GivenName', width: 130 },
    { field: 'OtherNames', headerName: 'OtherNames', width: 130 },
    { field: 'Gender_Type', headerName: 'Gender', width: 130 },
    { field: 'Date_Of_Birth', headerName: 'Date_Of_Birth', width: 130 },
    { field: 'MaritalStatus_Type', headerName: 'MaritalStatus_Type', width: 130 },
    { field: 'Address_Name', headerName: 'Address_Name', width: 130 },
    { field: 'TelNumber', headerName: 'TelNumber', width: 130 },
    { field: 'Email', headerName: 'Email', width: 130 },
    { field: 'Id_Nin_Number', headerName: 'Id_Nin_Number', width: 250 },
    { field: 'Parent_Global_Id', headerName: 'Parent_Global_Id', width: 150 },
    { field: 'CreationDate', headerName: 'CreationDate', width: 150 },
    { field: 'Creator_Name', headerName: 'Creator_Name', width: 150 },
    { field: 'EditDate', headerName: 'EditDate', width: 150 },
    { field: 'Editor_Name', headerName: 'Editor_Name', width: 150 },
    { field: 'Land_Search_RegDate', headerName: 'Land_Search_RegDate', width: 150 },
    { field: 'excellfilepath', headerName: 'excellfilepath', width: 150 },
    { field: 'geoshapefilepath', headerName: 'geoshapefilepath', width: 150 }
    
];

let neighbourcolumn = [
    { field: 'username', headerName: 'landsearchregistrar', width: 150 },
    { field: 'Object_Id', headerName: 'Object_Id', width: 150 },
    { field: 'Global_Id', headerName: 'Global_Id', width: 150 },
    { field: 'Name_Of_Adjacent_Owner', headerName: 'Neighbours Name', width: 500 },
    { field: 'Parent_Global_Id', headerName: 'Parent_Global_Id', width: 150 },
    { field: 'CreationDate', headerName: 'CreationDate', width: 150 },
    { field: 'Creator_Name', headerName: 'Creator_Name', width: 150 },
    { field: 'EditDate', headerName: 'EditDate', width: 150 },
    { field: 'Editor_Name', headerName: 'Editor_Name', width: 150 },
    { field: 'Land_Search_RegDate', headerName: 'Land_Search_RegDate', width: 150 },
    { field: 'excellfilepath', headerName: 'excellfilepath', width: 150 },
    { field: 'geoshapefilepath', headerName: 'geoshapefilepath', width: 150 }
];

let witnesscolumn = [
    { field: 'username', headerName: 'landsearchregistrar', width: 150 },
    { field: 'Object_Id', headerName: 'Object_Id', width: 150 },
    { field: 'Global_Id', headerName: 'Global_Id', width: 150 },
    { field: 'Name_Of_The_Witness', headerName: 'Name_Of_The_Witness', width: 500 },
    { field: 'Parent_Global_Id', headerName: 'Parent_Global_Id', width: 150 },
    { field: 'CreationDate', headerName: 'CreationDate', width: 150 },
    { field: 'Creator_Name', headerName: 'Creator_Name', width: 150 },
    { field: 'EditDate', headerName: 'EditDate', width: 150 },
    { field: 'Editor_Name', headerName: 'Editor_Name', width: 150 },
    { field: 'Land_Search_RegDate', headerName: 'Land_Search_RegDate', width: 150 },
    { field: 'excellfilepath', headerName: 'excellfilepath', width: 150 },
    { field: 'geoshapefilepath', headerName: 'geoshapefilepath', width: 150 }
];

let inspectioncolumn = [
    { field: 'username', headerName: 'landsearchregistrar', width: 150 },
    { field: 'Object_Id', headerName: 'Object_Id', width: 150 },
    { field: 'Global_Id', headerName: 'Global_Id', width: 150 },
    { field: 'Parent_Global_Id', headerName: 'Parent_Global_Id', width: 150 },
    { field: 'CreationDate', headerName: 'CreationDate', width: 150 },
    { field: 'Creator_Name', headerName: 'Creator_Name', width: 150 },
    { field: 'EditDate', headerName: 'EditDate', width: 150 },
    { field: 'Editor_Name', headerName: 'Editor_Name', width: 150 },
    { field: 'Land_Search_RegDate', headerName: 'Land_Search_RegDate', width: 150 },
    { field: 'excellfilepath', headerName: 'excellfilepath', width: 150 },
    { field: 'geoshapefilepath', headerName: 'geoshapefilepath', width: 150 }
];

let rptformcolumn = [
    { field: 'username', headerName: 'landsearchregistrar', width: 150 },
    { field: 'Object_Id', headerName: 'Object_Id', width: 150 },
    { field: 'Global_Id', headerName: 'Global_Id', width: 150 },
    { field: 'Parent_Global_Id', headerName: 'Parent_Global_Id', width: 150 },
    { field: 'CreationDate', headerName: 'CreationDate', width: 150 },
    { field: 'Creator_Name', headerName: 'Creator_Name', width: 150 },
    { field: 'EditDate', headerName: 'EditDate', width: 150 },
    { field: 'Editor_Name', headerName: 'Editor_Name', width: 150 },
    { field: 'Land_Search_RegDate', headerName: 'Land_Search_RegDate', width: 150 },
    { field: 'excellfilepath', headerName: 'excellfilepath', width: 150 },
    { field: 'geoshapefilepath', headerName: 'geoshapefilepath', width: 150 }
];


export {
    cldcolumn,
    ownercolumn,
    neighbourcolumn,
    witnesscolumn, 
    inspectioncolumn,
    rptformcolumn};